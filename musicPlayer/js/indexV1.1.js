...
;(function (w, callback, undefined) {
    ...
    function Song(option) {
        ...
        this.type = option.type;                    // 歌曲类型
        this.lyric = option.lyric;                  // 歌词
        this.singer = option.name.split(" - ")[0];  // 歌手
        this.curTime = 0;                           // 播放进度
        this.path = config.url + option.name;       // 歌曲路径 (写死了)
    }
    Song.setPrototype(w.zp, {
        type: config.songType
    });
    ...
    Player.setPrototype(w.zp, {
        init: function (id, theme) {
            ...
            this.addSong = this.contain.querySelector("button[data-target='addSong']");
            this.addList = this.contain.querySelector("button[data-target='addList']");
            this.delList = this.contain.querySelector("button[data-target='delList']");
            this.showList = this.contain.querySelector("button[data-target='showList']");
            this.toggleList = this.contain.querySelector("button[data-target='toggleList']");
            this.showAllList = this.contain.querySelector("button[data-target='showAllList']");
        },
        // 创建并添加歌单
...
// ### CodeContinue...
            ...
            HTMLAudioElement.prototype[val] = function () {
                ...
                switch (val){
                    ...
                    case "load":
                        ...
                        this.setAttribute("src", song.path);
                        break;
                }
                tp.call(this);
            };
    ...
    // 添加歌曲列表事件
    kuGou.addList.addEventListener("click", audio.addList.bind(audio));

    // 添加歌曲事件
    kuGou.addSong.addEventListener("click", audio.addSongs.bind(audio));

    // 删除当前列表
    kuGou.delList.addEventListener("click", audio.delList.bind(audio.player));

    // 显示当前所有歌单事件
    kuGou.showAllList.addEventListener("click", audio.showAllList.bind(audio.player));

    // 显示当前的歌曲列表事件
    kuGou.showList.addEventListener("click", audio.getCurList.bind(audio));

    // 切换歌单
    kuGou.toggleList.addEventListener("click", audio.toggleList.bind(audio));
